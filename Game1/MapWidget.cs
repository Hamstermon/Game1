...
namespace Squared.Tiled
{
                    ...
                    case XmlNodeType.Element:
                        switch (name)
                        {
                            case "image":
                                ...
                                result.TransColor = new Color();
                                result.TransColor.R = (byte)Convert.ToUInt32(trans.Substring(0,2),16);
                                result.TransColor.G = (byte)Convert.ToUInt32(trans.Substring(2, 2), 16);
                                result.TransColor.B = (byte)Convert.ToUInt32(trans.Substring(4, 2), 16);
                                result.TransColor.A = (byte)Convert.ToUInt32("ff", 16);
                                Console.WriteLine(result.TransColor.R + " " + result.TransColor.G + " " + result.TransColor.B);
                                break;
                            case "tile":
    ...
    public class Layer
    {
        ...
        public Texture2D GetTileTexture(int tileNo, Tileset tileset, Texture2D tile)
        {
            ...
            ///Console.WriteLine("start size"+TileTextureData.Length);
            ...
            for (int c = tilehit.Top; c < tilehit.Bottom; c++)
            {
                ...
                {
                    //Console.WriteLine("ize" + r + (c * tilesheet.Width));
                    Color colorA = TileTextureData[r + (c * tilesheet.Width)];
                    if (colorA != tileset.TransColor)
                        test[count] = colorA;
                    else
                    {
                        colorA.A = (byte)Convert.ToUInt32("00", 16);
                        test[count] = colorA;
                    }
                    count++;
                }
...
namespace Game1
{
    public class MapWidget : Image
    {
        ...
        public void Init(string mapname, Game1 game, GraphicsDeviceManager man)
        {
            parent = game;
            g = man;
